...
def test_ba_bai_dung_ba_trang_thai_nhu_README():
    _, row = M.read_table(M.read_fm(CAM / "campaign.md")[1], "CONTENT")
    assert {d["content_id"]: d["status"] for d in row} == {
...
def test_bai_da_dang_co_URL_that_trong_bang():
    ...
    for col in ("web", "youtube", "facebook"):
        assert d[col].startswith("http"), f"cột {col} phải có URL — đây là chỗ mở lại bài"
...
def test_HTML_sinh_lai_KHONG_doi():
    ...
    moi = GIO.sub("", BV.html_campaign(BV.read_campaign(CAM)))
    assert cu == moi, "campaign.html đã trôi — chạy build_views.py --station ./examples"

    kenhs = [BV.read_channel(k) for k in __import__("studio_paths").channels(VD)]
...
def test_file_dem_dang_khop_content_md(tmp_path):
    ...
    for khoa in ("blog", "yt_desc", "fb_post", "fb_comment"):
        name = PP.LAYOUT[khoa]
        moi, cu = tmp_path / name, BAI / name
        assert moi.is_file(), f"gen_article không còn sinh ra {name} — neo đã đổi?"
        assert cu.is_file(), f"ví dụ thiếu {name}"
        ...
        if "{{" in a:
            ...
            assert kh.sub("§", a) == kh.sub("§", b),                 f"{name} lệch content.md ở phần ngoài link"
            continue
        assert a == b, f"{name} lệch content.md — chạy lại gen_article.py"
...
def test_Excel_vi_du_KHONG_bo_trong_cot_trang_thai():
    ...
    _, row = _M.read_table(_M.read_fm(CAM / "campaign.md")[1], "CONTENT")
    queue = EX._dong_post(CAM, row)
    assert queue, "phải có dòng Post cho bài đã đăng"
    for o in queue:

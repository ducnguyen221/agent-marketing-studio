...
cột `publish_link` của sheet Post. Đây là khoá nối duy nhất về sau.

### Nếu KHÔNG cấp được quyền
Vẫn chạy được bằng đường xuất tay: Meta Business Suite → Insights → Nội dung → Xuất CSV.
File có cột **`ID bài viết`** — chính là khoá nối. Nối về bài qua `publish_link` của sheet Post.
...
### Mã bài để đối soát
`videoId` 11 ký tự. Ghi vào cột `publish_link` của sheet Post.
...
```
① Đăng xong          → ghi publish_link + publish_status vào sheet Post
...
# bất kỳ, nhập vào các cột actual_* của sheet Post, rồi bỏ. Không cần chỗ lưu cố định.
```
...
4. Đã chạy thử dry-run và kiểm sheet Post thấy đúng ý.
